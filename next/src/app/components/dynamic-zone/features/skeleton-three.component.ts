...
/**
 * SkeletonThree - Graph feature skeleton.
 * Replaces framer-motion animate with CSS animation for the gradient line
 * and a CSS scale animation for the message popup.
 */
@Component({
  selector: 'app-skeleton-three',
  standalone: true,
  styles: [`
    ...
    :host:hover .message-anim {
      animation: messagePopIn 0.4s ease-out forwards;
    }
    .gradient-animate stop {
      animation: gradLineMove 4s linear infinite;
    }
    @keyframes gradLineMove {
      0% { stop-color: #001AFF; }
      50% { stop-color: #6DD4F5; }
      100% { stop-color: #001AFF; }
    }
  `],
  template: `
    <div class="overflow-hidden h-full relative">
      <div class="flex justify-between gap-4">
        <div class="flex flex-col gap-1">
          <span class="font-medium">6,092</span>
          <span class="text-xs text-neutral-500">Last Month</span>
        </div>
        <div class="flex flex-col gap-1">
          <span class="font-medium">72K</span>
          <span class="text-xs text-neutral-500">Modules delivered</span>
        </div>
      </div>
      <div class="flex flex-col gap-4 items-center justify-center h-full relative">
        <!-- Message popup -->
        <div class="message-anim z-40 absolute left-10 top-10 rounded-full px-4 py-2 shadow-[0px_0px_8px_0px_rgba(248,248,248,0.25)_inset,0px_32px_24px_-16px_rgba(0,0,0,0.40)_inset]">
          <p class="text-xs">
            <span class="border rounded-md px-1 py-0.5 mr-1 border-indigo-500 bg-indigo-500/10 text-white">+8,008</span>
            launched
          </p>
        </div>
        <div class="p-8 relative">
          <div class="absolute inset-y-0 h-full w-20 right-16 z-30 bg-gradient-to-r from-transparent via-[#121213] via-[70%] to-[#121213]"></div>
          <div class="absolute inset-y-0 h-full w-20 left-16 z-30 bg-gradient-to-l from-transparent via-[#121213] via-[70%] to-[#121213]"></div>
          <!-- Main graph -->
          <svg width="335" height="163" viewBox="0 0 335 163" fill="none" xmlns="http://www.w3.org/2000/svg" class="text-neutral-600">
            ...
            <path d="M335 151L317.491 36.2214C317.166 34.0879 316.477 32.0245 315.57 30.0659C307.713 13.0898 308.853 1 284 1C257.738 1 244.262 37.1622 218 37.1622C191.738 37.1622 195.262 67.5 169 67.5C142.738 67.5 141.262 37.1622 115 37.1622C88.7381 37.1622 88.7141 76.5675 62.4522 76.5675C36.1902 76.5675 36.1902 54.6756 9.9283 54.6756H0" stroke="url(#gradient-3)" stroke-width="1.5" />
            <defs>
              <filter id="graph-line" x="-8" y="0.25" width="351.741" height="190.863" filterUnits="userSpaceOnUse" color-interpolation-filters="sRGB">
          ...
          <svg width="36" height="320" viewBox="0 0 36 320" fill="none" xmlns="http://www.w3.org/2000/svg" class="absolute inset-x-0 top-[4.2rem] h-1/2 w-full">
            ...
            <g filter="url(#filter0_bdi_1_60257)">
              <circle cx="18" cy="154" r="10" fill="#F8F8F8" fill-opacity="0.01" shape-rendering="crispEdges" />
              <circle cx="18" cy="154" r="10" fill="#121212" fill-opacity="0.3" shape-rendering="crispEdges" />
              <circle cx="18" cy="154" r="9.5" stroke="url(#paint0_linear_1_60257)" stroke-opacity="0.25" shape-rendering="crispEdges" />
            </g>
            <circle cx="18" cy="154" r="5" fill="#F8F8F8" />
            <defs>
              <filter id="filter0_bdi_1_60257" x="-4" y="132" width="44" height="72" filterUnits="userSpaceOnUse" color-interpolation-filters="sRGB">
                <feFlood flood-opacity="0" result="BackgroundImageFix" />
          ...
          <!-- Cursor -->
          <svg width="19" height="19" viewBox="0 0 19 19" fill="none" xmlns="http://www.w3.org/2000/svg" class="absolute inset-0 translate-x-4 m-auto h-4 w-4">
...
export class SkeletonThreeComponent {}
